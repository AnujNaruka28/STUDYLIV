...
const LecturesBox: React.FC<{ courseContent: Section[] }> = ({ courseContent }) => {
    ...
    const handleVideoClick = (courseId: string, subsectionId: string) => {
        ...
        // Find and set the lecture URL in context
        const currentSection = courseContent.find(section =>
            section.subSections.some(sub => sub.id === subsectionId)
        );
        const currentSubSection = currentSection?.subSections.find(sub => sub.id === subsectionId);
    ...
    return (
        <div className='w-full h-[80%] flex flex-col min-[1024px]:overflow-y-scroll'>
            {
